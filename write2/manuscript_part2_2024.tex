...
For an LES with isotropic grid and periodic horizontal boundaries, the turbulent structures of the vertical velocity component in the y-direction (spanwise near the surface) indicate how well the flow is resolved \citep{wurps2020grid}. Fig.\,\ref{fig:2pc_008} shows the two-point correlation $B_{ww}(y^*)$ at a height of $z^-\approx 0.08$, right inside the logarithmic layer. We define the average size of a structure in the flow $\sigma$ as the separation at which the two-point correlation drops to 0.3 (horizontal line in Fig.~\ref{fig:2pc_008}). The number of cells by which the average structure is resolved is equal to $\sigma/\Delta$. All cases show a similar behaviour, irrespective of the Reynolds number: the average size of the structure decreases with finer resolution and approaches a limit. The structure size of cases ReX\_150 and ReX\_200 agrees very well, which indicates beginning convergence. Furthermore, the number of cells by which the structures are resolved exceed 4 from the resolution of ReX\_150 on. This corresponds to the resolution from which on a logarithmic layer with a constant $\kappa$ can be seen. It also fits to the rule of thumb given in \cite{wurps2020grid}, that in a sufficiently resolved part of an LES the average structures of the w-component in y-direction should be resolved by at least 4 cells. The dynamic closure shows structures that are slightly smaller than the structures of $ReX\_200$, which is caused by a tendency to lower eddy-viscosities and, hence, a weaker coupling of neighboring grid cells.
