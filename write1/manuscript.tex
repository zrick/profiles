...
An alternative approach that considers viscous effects close to the surface is the van-Driest scaling \citep{vandriest:JOT1956}, 
...
For the higher layers of the ABL, the Ekman spiral is the simplest model available. When employed across the entirety of the ABL, the spiral is flattened, corresponding to a reduction of the veering angle both at the surface and throughout the ABL. We, however, find that a modified version of the Ekman spiral explicitly taking into account the surface boundary condition, is a consistent model and yields excellent 
agreement with the velocity profiles from DNS (Sec.~\ref{sec:ekman}). 

Given a surface veering, a formulation for the velocity profile in the logartihmic and Ekman layer can be obtained following the arguments by \cite{etling:2008}, cf. \cite{emeis:2018}. This simple model is compared to our DNS data and theoretical profiles in Fig.~\ref{fig:etling}. 
