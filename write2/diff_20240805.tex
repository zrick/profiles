...
%DIF DEL manuscript_part2_20240801_hw.tex   Mon Aug  5 11:22:45 2024
%DIF ADD manuscript_part2.tex               Mon Aug  5 11:18:56 2024
%DIF < \documentclass[smallcondensed,final]{svjour3}
%DIF -------
\documentclass[smallcondensed,draft]{svjour3} %DIF > 
%DIF -------

\usepackage{lmodern}
%DIF 4c4
%DIF < \usepackage[margin=2.5cm]{geometry}
%DIF -------
%\usepackage[margin=2.5cm]{geometry} %DIF > 
%DIF -------
\usepackage{booktabs} 
\usepackage{amsmath}
\usepackage{amssymb}
...
%DIF 18d20
%DIF < \hyphenation{Ol-den-burg}
%DIF -------

%DIF 20a21-22
\hyphenation{Ol-den-burg} %DIF > 
\hyphenation{non-sta-tio-na-ri-ty} %DIF > 
%DIF -------
...
\author{Hauke Wurps \DIFaddbegin \DIFadd{$^{1}$ }\DIFaddend and Cedrick Ansorge \DIFaddbegin \DIFadd{$^{2}$}\DIFaddend }
%DIF 34a37
\authorrunning{H. Wurps and C. Ansorge} %DIF > 
%DIF -------

%DIF 35a39-41
\institute{\DIFaddbegin \DIFadd{$^{1}$ Uni Oldenburg }\newline  %DIF > 
\DIFadd{$^{2}$ FU Berlin, Institut f\"ur Meteorologie, Carl-Heinrich-Becker-Weg 6--10, 12165 Berlin }\email{cedrick@posteo.de}\DIFaddend } %DIF > 
 %DIF > 
%DIF -------
\date{\DIFdelbegin \DIFdel{Draft }\DIFdelend \DIFaddbegin \DIFadd{Submitted }\DIFaddend \today}
\title{\DIFdelbegin \DIFdel{Towards a Universal Veering Profile for }\DIFdelend \DIFaddbegin \DIFadd{Wind veer in }\DIFaddend Turbulent Ekman Flow \DIFdelbegin \DIFdel{at arbitrary Reynolds number }\DIFdelend - Part 2\DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD <   \normalsize %%%
\DIFdel{LES and DNS of Turbulent Ekman Flow }%DIFDELCMD < \\%%%
\DIFdelend \DIFaddbegin \DIFadd{: Extrapolation to atmospheric scale separation}\DIFaddend }
\titlerunning{A Universal veering profile for Ekman flow - Part 2} %DIF > 
...
\begin{abstract}
  ...
  \DIFdelbegin \DIFdel{Turbulent Ekman flow is a canonical flow representation of the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend atmospheric boundary layer \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend characterized by a logarithmic increase of the wind speed \DIFdelbegin \DIFdel{near the surface }\DIFdelend \DIFaddbegin \DIFadd{in the surface layer }\DIFaddend and a turning of the wind vector aloft. \DIFdelbegin \DIFdel{In part I of this work, we derived a }\DIFdelend \DIFaddbegin \DIFadd{The simplest canonical configuration featuring both }\emph{\DIFadd{a logarithmic layer and the wind turning}} \DIFadd{is Ekman flow. A }\DIFaddend formulation of the mean \DIFdelbegin \DIFdel{wind velocity profiles of the stationary turbulent Ekman flow based on }\DIFdelend \DIFaddbegin \DIFadd{velocity profile was derived from scaling considerations and calibrated using }\DIFaddend direct numerical simulation \DIFdelbegin \DIFdel{(DNS)}\DIFdelend \DIFaddbegin \DIFadd{in part I of this work}\DIFaddend . Here, we explore the extrapolation \DIFdelbegin \DIFdel{of these profiles }\DIFdelend to atmospheric Reynolds \DIFdelbegin \DIFdel{numbers }\DIFdelend \DIFaddbegin \DIFadd{number (Re) }\DIFaddend using large-eddy simulation (LES).
  % Case desription and setup
  \DIFdelbegin \DIFdel{We compare theoretical }\DIFdelend \DIFaddbegin \DIFadd{Theoretical }\DIFaddend profiles of the wind vector \DIFdelbegin \DIFdel{for low Reynolds-number turbulent Ekman flow to results from DNS and LES. This necessitates the inclusion }\DIFdelend \DIFaddbegin \DIFadd{are compared to the theoretical reference at intermediate Re, which necessitates consideration }\DIFaddend of viscous effects \DIFdelbegin \DIFdel{, usually neglected in LES, }\DIFdelend %DIF >  [not required in abstract] (usually neglected in LES) 
  along with modifications to the standard bottom boundary condition.
  % Results and Discussion
  Analysis of the \DIFdelbegin \DIFdel{grid-convergence }\DIFdelend \DIFaddbegin \DIFadd{grid convergence }\DIFaddend for higher resolution unveils a convergence of LES data towards the theoretical profiles for \DIFdelbegin \DIFdel{both }\DIFdelend intermediate and high \DIFdelbegin \DIFdel{Reynolds numbers. Our }\DIFdelend \DIFaddbegin \DIFadd{Re. The }\DIFaddend LES thus confirm \DIFdelbegin \DIFdel{that---within the logarithmic layer (the lowest 10\,\% of the boundary layer) --- the direction }\DIFdelend \DIFaddbegin \DIFadd{that (i) the spanwise velocity scales as $\left.Re_\tau\right.^{-1}$, where $Re_\tau$ is the friction Reynolds number, and (ii) one third }\DIFaddend of the wind \DIFdelbegin \DIFdel{vector turns by approximately one-third of its total rotation across the entirety of the boundary layer.
  }\DIFdelend \DIFaddbegin \DIFadd{veer is confined to the surface layer.
  %DIF >  [this level of detail is not needed in the abstract] (the lowest 10\,\% of the boundary layer) 
  }\DIFaddend % Conclusions
  Such agreement of \DIFdelbegin \DIFdel{our }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend theoretical formulation and LES data raises \DIFdelbegin \DIFdel{our }\DIFdelend confidence in the underlying \DIFdelbegin \DIFdel{assumptions of the theory }\DIFdelend \DIFaddbegin \DIFadd{scaling assumptions }\DIFaddend and thus reinforces the utility of the theoretical profiles as a reference for intermediate and a quasi-reference for \DIFdelbegin \DIFdel{higher Reynolds number simulations}\DIFdelend \DIFaddbegin \DIFadd{both idealized simulation and field observation}\DIFaddend .
%
...
\DIFdelbegin \DIFdel{An important tool with capabilities to model the ABL flow is }\DIFdelend \DIFaddbegin 

\DIFaddend Large-Eddy Simulation (LES) \DIFdelbegin \DIFdel{. Indeed, LES }\DIFdelend is widely used to \DIFdelbegin \DIFdel{model }\DIFdelend \DIFaddbegin \DIFadd{study the }\DIFaddend turbulent flow in the atmospheric boundary layer (ABL) \DIFaddbegin \DIFadd{for a wide range of applications }\DIFaddend \citep{stoll2020large}. This includes \DIFdelbegin \DIFdel{various applications, such as simulating wind turbine }\DIFdelend \DIFaddbegin \DIFadd{the simulation of wind turbines }\DIFaddend and wind farm wakes, \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{their }\DIFaddend interactions within wind farm clusters \citep{porte2011large,mehta2014large,breton2017survey}. Furthermore, LES is applied in complex environments like mountainous or urban areas \citep{stoll2020large,garcia2018predictive} and to investigate the dispersion of pollution \citep{han2018large}. These applications typically focus on the lower segment of the ABL on the rotating earth, known as the Prandtl or surface layer. In this layer, the vertical wind speed profile commonly exhibits a logarithmic increase, followed by a prominent change of wind direction within the Ekman layer above. Accurate characterization of how wind speed and direction vary with height is of great importance for wind-power forecasting and projection \citep{optis2014moving}.

The simplest representation of the ABL taking into account rotation \DIFdelbegin \emph{\DIFdel{and}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend the quasi non-turbulent, free atmosphere aloft is \DIFdelbegin \DIFdel{the turbulent }\DIFdelend Ekman flow. It is a horizontally homogeneous, statistically stationary boundary-layer flow over a rotating flat surface. Here we consider the problem under neutral stratification, where \DIFdelbegin \DIFdel{the potential temperature is constant across the whole domain. Turbulent Ekman flow shows }\DIFdelend \DIFaddbegin \DIFadd{density effects on the flow due to temperature fluctuations are absent. This turbulent flow configuration features }\DIFaddend the key characteristics \DIFdelbegin \DIFdel{shaping }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend the real ABL\DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend a logarithmic layer and the Ekman spiral\DIFdelbegin \DIFdel{. Near }\DIFdelend \DIFaddbegin \DIFadd{; in immediate vicinity of }\DIFaddend the surface, viscous forces dominate \DIFdelbegin \DIFdel{the flow}\DIFdelend \DIFaddbegin \DIFadd{(}\emph{\DIFadd{viscous sublayer}}\DIFadd{)}\DIFaddend . The stationary ABL-height is \DIFdelbegin \DIFdel{then defined }\DIFdelend \DIFaddbegin \DIFadd{set }\DIFaddend by the interplay between turbulent growth due to the shear instability \DIFdelbegin \DIFdel{of the configuration }\DIFdelend \citep{lilly1966instability} and rotational suppression of turbulence due to the Coriolis effect. The system's statistical equilibrium is \DIFdelbegin \DIFdel{defined by one }\DIFdelend \DIFaddbegin \DIFadd{characterized by a }\DIFaddend single parameter, the Reynolds number $\RE$, signifying the scale separation between the largest and smallest \DIFdelbegin \DIFdel{scale within }\DIFdelend \DIFaddbegin \DIFadd{scales of }\DIFaddend the problem. The largest scales are given by shear-induced eddies constrained by the \DIFdelbegin \DIFdel{boundary layer's }\DIFdelend \DIFaddbegin \DIFadd{boundary-layer }\DIFaddend height, while the smallest scales reside within the dissipative range, where strong viscous forces suppress turbulence. The dimensional mean solution of the turbulent Ekman layer is influenced by three parameters: the geostrophic wind $G$, the Coriolis parameter $f$, and the kinematic viscosity $\nu$. These three parameters define the Reynolds number $Re_D = GD/\nu$. \DIFdelbegin \DIFdel{With }\DIFdelend \DIFaddbegin \DIFadd{Using }\DIFaddend $D=\sqrt{2\nu/f}$\DIFaddbegin \DIFadd{, }\DIFaddend the Ekman layer depth of a laminar flow\DIFdelbegin \DIFdel{we write
}%DIFDELCMD < 
...
%Definitions
We align the coordinate Ox with the surface stress \DIFaddbegin \DIFadd{(\mbox{%DIFAUXCMD
\cite{ansorge:BM2019}}\hspace{0pt}%DIFAUXCMD
...
\DIFdelbegin \DIFdel{We studied }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend truly neutral Ekman boundary \DIFdelbegin \DIFdel{layers---absent }\DIFdelend \DIFaddbegin \DIFadd{layer---absent }\DIFaddend of capping inversions, \DIFdelbegin \DIFdel{non-stationarity}\DIFdelend \DIFaddbegin \DIFadd{non-statio\-narity}\DIFaddend , and surface \DIFdelbegin \DIFdel{heterogeneity}\DIFdelend \DIFaddbegin \DIFadd{heterogeneity---is studied as a prototype representation of the ABL}\DIFaddend . For the first time, an LES of the Ekman layer is \emph{a posteriori} validated against a DNS for an identical configuration. Despite the common assumption that viscosity as a parameter drops out in LES as a consequence of the turbulence closure, we considered a range of scale separations (\DIFdelbegin \DIFdel{Reynolds numbers}\DIFdelend \DIFaddbegin \DIFadd{$\RE$}\DIFaddend ) and found that for an exact representation of the surface turning and wall friction as well as the wind-turning profile, viscous effects need consideration. We derive an analytical relation demonstrating that --- given fixed surface properties --- the specification of $z_0$ implicitly defines a viscosity. The LES suffer from the dilemma, that in Ekman flow, some aspects of crucial importance happen on relatively small scales, such as the rotation-surface interaction which is confined to the inner layer.

With the wind-profile formulation developed in Part 1 of this work, we \DIFdelbegin \DIFdel{have }\DIFdelend \DIFaddbegin \DIFadd{establish }\DIFaddend a reference based on first principles for \DIFdelbegin \DIFdel{intermediate-Re simulations}\DIFdelend \DIFaddbegin \DIFadd{intermediate $\RE$}\DIFaddend , and a quasi-reference for \DIFdelbegin \DIFdel{the higher-Re simulations}\DIFdelend \DIFaddbegin \DIFadd{geophysical $\RE$}\DIFaddend . While we acknowledge that both the LES and the wind-profile model suffer from assumptions for high \DIFdelbegin \DIFdel{Reynolds number}\DIFdelend \DIFaddbegin \DIFadd{$\RE$}\DIFaddend , their quantitative agreement across a wide range of scale separations hints towards \DIFdelbegin \DIFdel{the consistency. In particular, we understand that the }\DIFdelend \DIFaddbegin \DIFadd{consistency. The }\DIFaddend grid convergence of LES towards the theoretical profiles underpins the inviscid scaling hypotheses in the development of the theory underlying part 1 of this paper. \DIFaddbegin \DIFadd{This approach raises our confidence in the LES to a new level for the direct comparison with a resolving simulation. Furthermore, it allows us to extend the insight from DNS to atmospheric scale where an approach through DNS is clearly infeasible for computational constraints. 
}\DIFaddend 


We simulated three different Reynolds numbers while \DIFdelbegin \DIFdel{having DNS results }\DIFdelend \DIFaddbegin \DIFadd{DNS data is available }\DIFaddend for the lowest $\RE$ \DIFdelbegin \DIFdel{. For the low }\DIFdelend \DIFaddbegin \DIFadd{only. For this lowest }\DIFaddend $\RE$, viscous forces \DIFdelbegin \DIFdel{have a significant contribution }\DIFdelend \DIFaddbegin \DIFadd{significantly contribute }\DIFaddend to the balance of forces on the \DIFdelbegin \DIFdel{grid scale of the LES , hence }\DIFdelend \DIFaddbegin \DIFadd{LES grid, and }\DIFaddend we adapted the LES code to consider the \DIFdelbegin \DIFdel{fluid's }\DIFdelend \DIFaddbegin \DIFadd{molecular fluid }\DIFaddend viscosity in addition to the modeled eddy-viscosity. We did not introduce a roughness length $z_0$ at the lower boundary as additional parameter but deduced $z_0$ according to the law of the wall and the shear velocity expected according to the semi-empirical law by \cite{spalart1989theoretical}. The interplay of geostrophic wind, shear velocity, and roughness length in the simulation showed remarkable consistency, which supports the value of the adapted boundary condition at the bottom \citep{maronga2017formulation}. The dependence of the LES solution on the grid cell size was investigated through a comparison of four different resolutions. The setups of all $\RE$ use similar grid sizes in terms of the outer scale ($\Delta^-=\Delta/\delta=const.$) but different grid sizes in terms of the inner scale ($\Delta^+=\delta^+\cdot const. = Re_\tau\cdot const.$). This means that from an inner scale perspective the high Reynolds numbers were much less well resolved.
...
When these considerations are taken into account, LES becomes possible at uncommonly low Reynolds number and  \DIFaddbegin \DIFadd{high }\DIFaddend resolution, which allows a quantitative comparison to state-of-the art DNS. \DIFaddbegin \DIFadd{Most importantly, we observe a clear $\RE$-dependency in the LES results, matching the predictions of the theoretical framework, illustrating that, also in  LES, the choice of $z_0$ implicitly defines a $\RE$.  
}\DIFaddend 
...
The simulations were performed at the HPC Cluster EDDY, located at the University of Oldenburg (Germany) and funded by the Federal Ministry for Economic Affairs and Energy (Bundesministeriums für Wirtschaft und Energie) under grant number 0324005. \DIFaddbegin \DIFadd{The DNS were generated on the HPC systems }\texttt{\DIFadd{juwels}} \DIFadd{at J\"ulich Supercomputing Centre under the (grants }\texttt{\DIFadd{stadit}}\DIFadd{, }\texttt{\DIFadd{hku24)}} \DIFadd{and }\texttt{\DIFadd{hawk}} \DIFadd{at H\"ochstleistungs\-rechen\-zentrum Stuttgart (Bundesprojekt }\texttt{\DIFadd{trainABL}}\DIFadd{, \#44187). CA acknowledges funding by the European Commission through ERC Starting Grant \#851374. We are thankful to the refubium team of FU Berlin for making available the DNS data used in this publication through their repository. 
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \bibliographystyle{plainnat}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibliographystyle{spbasic_updated}
\DIFaddend \bibliography{papers.bib,ansorge.bib}
