...
\titlerunning{A Universal Velocity Profile for Ekman Flow}
...
obtained. 
% 
While the K-Profiles concern the total stress, a consistent formulation of the turning would also require its partitioning to the individual components, i.e. 
the orientation of the stress vector $(\overline{u'w'}, \overline{v'w'})$ in the horizontal plane. If K-theory shall be used, this stress vector needs to be anti-parallel to the corresponding stress vector $(\partial_z U, \partial_z V)$. 
% 
Fig.~\ref{fig:alignment} shows the direction of the velocity, gradient and stress 
vectors across the boundary layer. It turns out that the negative stress vector with respect to the wind direction and the flux vectors (absolute) have an approximately similar direction. 
%
...
%
However, this implies the a misalignment between the flux and the stress on the order of the wind turning, and indeed Fig.~\ref{fig:alignment}b shows a misalignment up to $90^\circ$. This is in accordance with the expectation by \cite[][\S 7.18]{Townsend:1976} and prevents the transfer of Energy from the mean flow to turbulence at these heights, thus preventing a boundary-layer growth. 
% 
While the behavior in the inner layer seems to depend on $Re$, there emerges universality in the misalignment across the outer layer, suggesting that a universal consideration of misalignment in the context of K-theory is possible for formulations of higher-order quantities. 
%
\begin{figure}
  \textbf{(a)\hspace{0.48\textwidth}(b)}\\
...
\section{Conclusions}
%
We investigate the wind veer in Ekman flow. Based on scaling considerations and direct numerical simulation spanning on decade in external separation from $Re_\Lambda=1.25 \times 10^5$ to $Re_\Lambda=1.28\times 10^6$, we derive a universal formulation for the both horizontal components of the velocity profile in a stationary, smooth Ekman layer.  
% 
This formulation is consistent with the DNS data and also yields reasonable results 
at geophysical scale separation; the logarithmic law of the mean velocity is recovered with the well-known limits and deviations towards the surface and Ekman layer. 
%
...
\par
% 
To quantify the spanwise velocity component consistently across the boundary layer, 
we derive a universal scaling of the spanwise velocity component in a shear-aligned reference frame. For the \emph{inner layer}, we find the mixed scaling  
...
% 
In the outer layer, the spanwise velocity follows outer scaling, consistently with the Ekman model mentioned above (Sec.~\ref{sec:ekman}).
% 
\par
%
The velocity profiles are shown in inner and outer scaling as well as in ho\-do\-graph- view in Fig.~\ref{fig:summary}. The shear-aligned velocity deficit is shown in outer scaling highlighting the universality in the outer layer. 
% 
In the inner layer, the logarithmic profile (blue, panel \textbf{(b)}) is ecnountered across a widening region for increasing Reynolds number. 
% 
...
% 
While, at a fixed height, the spanwise velocity scales approximately as $Re_\tau^{-1}$, the fraction 
of turning that is encountered within the inner layer of the flow amounts to about 1/3 of the total wind veer 
(Fig.~\ref{fig:etling}a).
% 
The hodographs show the well-known Ekman shape with the laminar profile as an outer limit and 'flatter' 
hodographs, corresponding to less turning, for increasing Reynolds number. 
% 
Importantly, our results suggest that there is no lower limit of the turning. 
% 
This implies that, despite its very large scale separation and huge Reynolds number, the ABL cannot be 
in the 'limit' of high Reynolds number from the perspective of wind veer.  
