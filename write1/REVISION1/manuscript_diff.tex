...
%DIF DEL manuscript_orig.tex   Thu Mar 27 17:47:33 2025
%DIF ADD manuscript.tex        Mon Mar 24 18:16:05 2025
\smartqed
\usepackage{appendix}
\usepackage{amsmath}
\usepackage{graphicx}
...
\usepackage{hyperref}
%DIF 17a17
\journalname{Bounday-Layer Meteorology} %DIF > 
%DIF -------
\setcitestyle{aysep={}}
\linenumbers 
...
\title{Wind veer and speed in turbulent Ekman flow part I: scaling analysis and \DIFdelbegin \DIFdel{universal }\DIFdelend \DIFaddbegin \DIFadd{velocity }\DIFaddend profile model}
%DIF 64c65
%DIF < \titlerunning{A Universal Velocity Profile for Ekman Flow}
%DIF -------
\titlerunning{Scaling Analysis and Velocity Profile for Ekman Flow} %DIF > 
%DIF -------
...
\begin{abstract}
  ...
  %
  The profile of the streamwise component\DIFaddbegin \DIFadd{, considered in a wall-stress-aligned reference frame, }\DIFaddend follows the classical viscous, logarithmic and wake scaling.
  %
  ...
  becomes universal as a function of \DIFdelbegin \DIFdel{outer height}\DIFdelend \DIFaddbegin \DIFadd{the outer height, commonly denoted as $z^-$}\DIFaddend .
  %
  This implies a mixed scaling for the turbulent stress and eddy viscosity across the inner layer
  and convergence to a \DIFdelbegin \DIFdel{universal }\DIFdelend scaling as function of the outer height across the outer layer
  for increasing scale separation\DIFdelbegin \DIFdel{vide Reynolds numbers}\DIFdelend \DIFaddbegin \DIFadd{, i.e. for increasing Reynolds number}\DIFaddend . 
  % 
...
\cite{etling:2002, emeis:2018}  (Sec. 21.10; Eq. 21.48); they choose the Prandtl-layer \DIFdelbegin \DIFdel{height $z_\text{Prandtl}$ }\DIFdelend \DIFaddbegin \DIFadd{depth, 
a measure for the depth of the surface layer, }\DIFaddend to match the wind speed profiles\DIFdelbegin \DIFdel{, which, however, }\DIFdelend \DIFaddbegin \DIFadd{. 
However, this }\DIFaddend requires external prescription of \DIFdelbegin \DIFdel{$\alpha(z_\text{prandtl})$, }\DIFdelend the veering at that height. 
...
homogeneous, stationary ABL over a flat surface \DIFaddbegin \DIFadd{and }\DIFaddend under neutral stratification. 
%
\DIFdelbegin \DIFdel{Universal profiles of the wind vector for turbulent Ekman flow not only are }\DIFdelend \DIFaddbegin \DIFadd{The description of velocity profiles for this strongly simplified problem is not only a canonical fluid-mechanical problem, but it 
also constitutes }\DIFaddend a well-described limit for theoretical exploration or higher-order approaches taking into account possible effects of stratification,
...
%
(iv) Barotropic and baroclinic effects on the velocity profile require \DIFdelbegin \DIFdel{to consider 
}\DIFdelend \DIFaddbegin \DIFadd{consideration of
}\DIFaddend the height-dependence of the veer and stress misalignment \citep{momen:JAS2018, ghannam:QJR2021}.
% 
...
%
The f-plane approximation is applied\DIFdelbegin \DIFdel{such that rotation only acts on horizontal velocity components ;
we thus neglect }\DIFdelend \DIFaddbegin \DIFadd{, i.e. rotation acts on the horizontal components of velocity alone;
}\DIFaddend rotational effects on the \DIFdelbegin \DIFdel{horizontal components }\DIFdelend \DIFaddbegin \DIFadd{vertical component }\DIFaddend of velocity and dynamical effects
due to latitudinal variation of the rate of rotation \DIFaddbegin \DIFadd{are neglected}\DIFaddend .
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
correspond to an \DIFdelbegin \DIFdel{Ensemble }\DIFdelend \DIFaddbegin \DIFadd{ensemble }\DIFaddend average. 
%
...
\begin{subequations}
\begin{equation}
  \underline{\tau}_\mathrm{sfc} = \left(\begin{array}{c} \tau_x\\ \tau_y\\\tau_z\end{array}\right) = - \DIFaddbegin \DIFadd{\rho}\DIFaddend \nu \left( \frac{\partial U}{\partial z} \hat{e}_x + \frac{\partial V}{\partial z} \hat{e}_y\right)
\end{equation}
...
\begin{equation}
  u_\star = \DIFdelbegin \DIFdel{\sqrt{ || - \underline{\tau}_\mathrm{sfc} || } 
  }\DIFdelend \DIFaddbegin \DIFadd{\sqrt{ \frac{|| - \underline{\tau}_\mathrm{sfc} ||}{\rho} } 
  }\DIFaddend \label{eqn:ustar_def}
...
the only non-dimensional parameter that governs the system \DIFaddbegin \DIFadd{(in Ekman flow, the ratio of Rossby and Reynolds number 
are not independent as $Re_D=2\Lambda_\Ro/D$)}\DIFaddend .
%
...
scales ($u_\star, \nu$). 
%
\DIFaddbegin \DIFadd{The outer normalization (with respect to the boundary-layer depth $\delta$ and velocity $u_\star$) is denoted 
by a superscript $'-'$, i.e. $z^-=z/\delta$.  
%DIF > 
}\DIFaddend Another common measure of scale separation is the Reynolds number 
\begin{equation}
  \RE_D = \frac{G D}{\nu} 
...
\par
%
The \DIFdelbegin \DIFdel{Eddy }\DIFdelend \DIFaddbegin \DIFadd{eddy }\DIFaddend viscosity plays a crucial part when modelling profiles and the vertical
...
on the strain rate.
%
For the \DIFdelbegin \DIFdel{symmetries in the flow (horizontal homogeneity , and }\DIFdelend \DIFaddbegin \DIFadd{assumptions of horizontal homogeneity and incompressibility (which implies  }\DIFaddend $W=0$\DIFaddbegin \DIFadd{, i.e. no mean wall-normal velocity}\DIFaddend ), it is
%
\begin{subequations} 
\begin{equation}
...
\begin{figure}
  ...
  \caption{Horizontal slices of turbulence kinetic energy in the
    Buffer layer (i=10\DIFaddbeginFL \DIFaddFL{, $z^+\approx 10.5$}\DIFaddendFL ),
    logarithmic \DIFdelbeginFL \DIFdelFL{Layer }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{layer }\DIFaddendFL (i=100\DIFaddbeginFL \DIFaddFL{, $z^+ \approx 150$}\DIFaddendFL ), and
    outer layer (i=400\DIFaddbeginFL \DIFaddFL{, $z^+ \approx 1200$}\DIFaddendFL ) of the case with $Re_\tau=2978$;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{A \DIFdelbegin \DIFdel{universal }\DIFdelend \DIFaddbegin \DIFadd{Reynolds-number-independent }\DIFaddend velocity profile for the turbulent Ekman layer}
\label{sec:profiles} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
\label{sec:drag-law}
\begin{figure}
  \centerline{\includegraphics[width=0.7\textwidth]{Fig4.eps}} % ustar_alpha.eps}}
...
boundary conditions at the surface but at a different height \DIFaddbegin \DIFadd{while maintaining the restriction 
to constant eddy viscosity}\DIFaddend . 
%
...
%
\begin{figure}
  \centerline{\includegraphics[width=0.5\textwidth]{Fig9.eps}} %c_log.eps}}
...
\begin{align}
  \omega(z^-) = \frac{1}{2} \left[\text{erf}\left(z_\text{trans}\log\frac{z^-}{z_{\text{blend}}^{-}}\right) +1\right],
\end{align}
hence 
...
Eq.~(\ref{eqn:scaling_spanwise}) establishes a \DIFdelbegin \DIFdel{universal }\DIFdelend mixed scaling for the spanwise velocity in the viscous layer:
%
...
%%% VAN DRIEST SCALING
\begin{figure}
  \centerline{\includegraphics[width=0.66\textwidth]{Fig11.eps}} %vanDriest_profile.eps}}
...
%
Based on scaling considerations and direct numerical simulation spanning on decade in external separation from $Re_\Lambda=1.25 \times 10^5$ to $Re_\Lambda=1.28\times 10^6$, we derive a \DIFdelbegin \DIFdel{universal formulation for the }\DIFdelend \DIFaddbegin \DIFadd{closed formulation for }\DIFaddend both horizontal components of the velocity profile in a stationary, smooth Ekman layer.  
% 
