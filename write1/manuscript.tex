...
\begin{equation}
  U_\text{inner}^{\alpha_\star}^+ = f_{U,\mathrm{inner}}(z^+) = \frac{ z^+ + \gamma_4 (z^+)^{4} + \gamma_6 (z^+)^{6} } { 1 + \gamma_6 /u_{ref} (z^+ )^6}, 
\end{equation} 
...
If the difference across the boundary layer is constant ($C_{V0}$) across variations of $\RE$,
the averaged gradient $\partial_{z^+} V^{\alpha_\star+}$ of the spanwise velocity component
must decrease as $1/\delta^+$ as $Re_\tau$ increases.
%
...
\begin{equation}
  V^{\alpha_\star}\frac{\delta^+}{G} = f_{V,\mathrm{inner}}(z^+) = v_\mathrm{ref} \left( \omega_v z^+ -1 + \exp[-\omega_v z^+]\right),
...
inversely proportional to the friction Reynolds number $\RE_\tau=\delta^+$ when considered at a fixed height.
%
...
the spanwise velocity is a higher-order correction term from the view point of similarity theory and
the channel-flow analogy that is routinely employed in the surface layer.
%
Further, this is consistent with the scaling for the velocity hodograph found in Eq.~(\ref{eqn:hodograph_scaling}) where
the friction velocity also drops out.
%
\par
%
The universal scaling of $V^{\alpha_\star}/G\delta^+$ continues through the surface layer,
and we find a very similar picture to the spanwise component with an increasing
depth of a quasi-logarithmic layer as shown in Fig.~\ref{fig:inner_w}b.
%
We model this layer by a profile of the form
%
\begin{equation}
  f_{V,\mathrm{log}}(z^+) = \frac{V_\mathrm{log}(z^+)}{G} \delta^+ = a_\mathrm{log} + b_\mathrm{log} \log z^+ + c_\mathrm{log} z^+
\end{equation} 
%
This profile is constrained by
\begin{align}
  f_{V,\mathrm{log}}(z^+=10) &=& f_{V,\mathrm{inner}}(z^+=10) &=:& v_{10} \simeq 27.3\\
  \frac{\p}{\p z^+} \left[ f_{V,\mathrm{log}}\right]_{z^+=10} &=& \frac{\p}{\p z^+}\left[ F_{V,\mathrm{inner}}\right]_{z^+=10} &=:&d_{10}\simeq 4.01 \\
  f_{V,\mathrm{log}}(z^+=0.3\delta^+) &=& V^{\alpha_\star}_\mathrm{ek}(z^-=0.3)\delta^+&=:& v_{03} 
\end{align}
where $v_{03}$ is determined by $V_\mathrm{ek}(0.3)$ and $U_\mathrm{ek}(0.3)$ and depends on the Reynolds number. 
